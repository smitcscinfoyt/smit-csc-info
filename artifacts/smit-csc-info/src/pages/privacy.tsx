...
import { PRIVACY_DOCS, type LegalSection } from "@/lib/legal-content";

export default function Privacy() {
  const { language } = useLanguage();
  const isPrime = useIsPrime();
  const doc = PRIVACY_DOCS[language] ?? PRIVACY_DOCS.en;
  ...
  return (
    <div className={isPrime ? "bg-gradient-to-b from-purple-50/40 via-white to-amber-50/30" : ""}>
          ...
          <div className="container mx-auto max-w-4xl relative z-10">
            ...
            <div className="flex items-center gap-3">
              <div className="h-12 w-12 rounded-xl bg-gradient-to-br from-amber-400 to-yellow-600 flex items-center justify-center shadow-lg">
                <ShieldCheck className="h-6 w-6 text-purple-950" />
              </div>
              <div>
                <h1 className="text-3xl md:text-4xl font-black text-white" data-testid="privacy-title">{doc.title}</h1>
      ...
      <div className="container mx-auto px-4 py-12 max-w-4xl">
        {!isPrime && (
          <>
            <h1 className="text-3xl font-bold text-primary mb-2" data-testid="privacy-title">{doc.title}</h1>
