...
import { SubscribeMembershipBody, GetMembershipPlansResponse, GetMembershipStatusResponse, SubscribeMembershipResponse } from "@workspace/api-zod";
...
router.get("/membership/status", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  ...
  // Days left in grace, rounded UP so users see "1 day left" through the final 24h,
  // and "X days left" reflects time remaining (not full days elapsed).
  ...
  res.json(
    GetMembershipStatusResponse.parse({
      isActive: status.isActive || status.isInGracePeriod, // grace = still has access
...
router.post("/membership/subscribe", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  ...
  const [user] = await db.select({ mobile: usersTable.mobile }).from(usersTable).where(eq(usersTable.id, userId)).limit(1);
  ...
  await db.insert(paymentsTable).values({
    userId,
    amount: Math.ceil(finalPaise / 100),
    plan: plan.id,
    transactionId,
    status: "pending",
    ...
    discountPaise,
    originalAmountPaise: plan.price,
  });
  ...
  // Free after 100% coupon → not supported for prime (PhonePe gateway minimum). Reject.
  if (finalPaise === 0) {
    res.status(400).json({ error: "Prime cannot be 100% free via coupon. Please choose a smaller discount." });
    return;
  }

  try {
    const base = getCallbackBaseUrl();
    // After payment, PhonePe redirects user browser to this URL.
    // Our backend then checks payment status and redirects to frontend.
    ...
    res.status(502).json({
      ...
      hint: err.message?.includes("SU") || err.message?.includes("sandbox")
